...
class CustomerAuthController extends Controller
{
    ...
    {
        // Skip validation to avoid any blocking - just accept whatever Firebase sends
        $phone = $request->phone;
        ...
        if ($user) {
            // User exists, login them - just mark as verified and return token
            $user->is_phone_verified = 1;
            $user->save();
            ...
            // User doesn't exist, create new user with minimal info
            $user = new User();
            $user->phone = $phone;
            $user->password = bcrypt($phone); // Simple password for food delivery app
            $user->is_phone_verified = 1;
            $user->login_medium = 'otp';
            $user->save();
